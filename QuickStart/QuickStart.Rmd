...
## Identification genes with affected expression
...
* `test` - character defining test. Possible values of parameter `test` are described in function documentation.

Tests are based on the methods implemented in packages `Deseq`, `Deseq2` and `edgeR`.
...
* `test1`, `test2` - characters defining tests. Possible values of parameter `test` are described in `calculate_test()` function documentation. 

```{r, message=FALSE, warning=FALSE}

comparison<-calculate_comparison_table(data1 = BRCA_exp[,!(colnames(BRCA_exp)=="SUBTYPE")], 
                                               data2 = BRCA_met_gen, 
...
* `data.m`, `data.e` - data sets with informations from methylation and expression respectively. In the example below, we use the `BRCA_met` and `BRCA_exp` data without the `SUBTYPE` column.
...
* `test.m`, `test.e` - results from `calculate_test()` function for methylation an expression data.
