...
visual_volc <- function(condition.e, condition.m, data.m, data.e, gene, test.e, test.m){
  ...
  for(i in 1:max(l.e, l.m)){
    ...
    plist[[length(plist)+1]] <- volcano_plot(test.e[[i]], ngen = gene,ylog=TRUE,title="",fold_line = 2, line=0.05)
    }
    ...
    plist[[length(plist)+1]] <- volcano_plot(test.m[[i]], ngen = gene, title="", ylog=TRUE, line=0.05)
    }
