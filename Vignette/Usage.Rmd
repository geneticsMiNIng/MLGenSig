...
author: "Aleksandra DĂ„â€¦browska, Alicja Gosiewska"
date: '`r Sys.Date()`'
output:
...
library(ggplot2)
library(edgeR)
condition.e <- ifelse(BRCA_mRNAseq_chr17[,1]=="LumA","LumA","other")
condition.m <- ifelse(BRCA_methylation_chr17$SUBTYPE=="LumA","LumA", "other")
g <- genereg_vs_met(BRCA_methylation_chr17, condition.m, "BRCA1", observ = TRUE, show_gen = TRUE)
#normalization * 10000
BRCA_mRNAseq_chr17_cpm <- cpm(BRCA_mRNAseq_chr17[,-1]) 
b1 <- boxplot_that(BRCA_mRNAseq_chr17_cpm,"BRCA1",condition.e)
v.e <- volcano_plot(test.expr, ngen = "BRCA1",ylog=TRUE, title="expression", fold_line = 2, line=0.05)


BRCA_methylation_chr17_gen <- map_to_gene(BRCA_methylation_chr17[,-1])
test.mety <- test_diff(BRCA_methylation_chr17_gen, condition.m, test="ttest")
v.m <- volcano_plot(test.mety,ngen = "BRCA1",ylog=TRUE,title="methylation",line=0.05)

s <- tableGrob(t(sum_gen(BRCA_mRNAseq_chr17[,-1],condition.e , "BRCA1")))
...
visual_gene(condition.e, condition.m, BRCA_methylation_chr17[,-1],BRCA_mRNAseq_chr17[,-1], "BRCA1", test.expr, test.mety)


visual_volc(condition.e, condition.m, BRCA_methylation_chr17[,-1],BRCA_mRNAseq_chr17[,-1], "BRCA1", list(nbinom =test.expr), list(ttest=test.mety))
```

```{r, message=FALSE, fig.width=19, fig.height=20}
visual_volc(condition.e, condition.m, BRCA_methylation_chr17[,-1],BRCA_mRNAseq_chr17[,-1], "BRCA1", list(nbinom=test.expr,nbinom2=test.expr), list(ttest=test.mety,test.mety))
...
```
 ...
 ```{r, message=FALSE, fig.width=19, fig.height=20}
# visual_volc(condition.e, condition.m, BRCA_methylation_chr17[,-1],BRCA_mRNAseq_chr17[,-1], "BRCA1", list(nbinom=test.expr,nbinom2=test.expr), list())
# 
# 
 ```